...
function accountTable() {
  const head = 'name | role | off_map_prospect | dragonfly_status | jb_tier | jb_target_tier | dfly_value_tier | fpr_sales_90d_usd | fpr_avg_price | fpr_velocity_vs_mkt | pistil_decile | store_rank_90d | sales_90d_usd | sales_30d_usd | momentum_30v90_pct | trend_90v180_pct | momentum_vs_market_pct | days_since_order | hist_rev_usd | city | neighborhood | county | region | rep | poc | phone | license | lat | lng';
  const lines = ACCOUNTS.map((d) => [
    d.n, d.role, d.prospect ? 'yes' : '', num(d.ds), num(d.tier), num(d.jbt), num(d.dft),
    num(d.fpvol), num(d.fpprice), num(d.fpvel),
...
TWO BRANDS, OPPOSITE ENDS (data-driven flower/preroll fit — use for product placement):
- Jerome Baker (JB) = PREMIUM glass + premium flower/preroll (7g jars ~$65-75 retail, 1g prerolls ~$13-14). jb_target_tier (1=best,2,3) flags stores where premium flower/preroll sells well (high fpr_avg_price + volume + velocity). For "where should JB go / premium targets", rank by jb_target_tier then fpr_sales_90d; favor jb_target_tier=1 that are New Prospects (whitespace).
- Dragonfly (Dfly) = VALUE, cheapest in every category (~$6 preroll / $20 3.5g retail). dfly_value_tier (1=best) flags high-UNIT, value-priced flower/preroll stores where Dragonfly's cheap line moves. For "where should Dragonfly expand", rank by dfly_value_tier + units; Dragonfly Fallow/Slipping value stores are reactivation plays.
- fpr_sales_90d_usd / fpr_avg_price = flower+preroll sales and avg price over 90d. fpr_velocity_vs_mkt = the store's flower/preroll momentum minus the market median (negative ≈ tracking a cooling category; positive = outperforming). JB and Dragonfly are UNRELATED beyond shared ownership — never imply one is the other's tier.
